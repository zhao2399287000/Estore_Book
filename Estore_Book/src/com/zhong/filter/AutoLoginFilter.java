...
import com.zhong.service.impl.UsersServiceImpl;
import com.zhong.utils.CookieUtils;

/**
 * filter:过滤器 每一个过滤器类应该实现filter接口 可以使用过滤器拦截所有的请求， 对请求进行验证,如果请求合法，放行，如果不合法，进行拦截
 * 
 * 权限的控制，登陆认证，日志
 * 
 */
public class AutoLoginFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
			...
			if (users == null) {
				...
				if (cookie != null) {
					...
					if (uri.indexOf("order") != -1 || uri.indexOf("Order") != -1) {
						// 跳转到登陆页面
						// 重定向
						response.sendRedirect("showLogin?type=4");
						return;
		...
		if (uri.indexOf("admin") != -1 || uri.indexOf("Admin") != -1) {
			// 跳转到登陆页面
			// 重定向
			response.sendRedirect("showLogin?type=4");
			return;
