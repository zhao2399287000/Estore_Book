...
<body>
	...
	<div class="estore_content" style="min-height: 800px;">
		...
		<form action="reg.action" method="post" onsubmit="return checkform()"
			id="registform">
			...
			<table>
				<tr>
					...
					<td><input type="text" name="username" id="username"
						onblur="lostfocus()"> <span id="username_span"></span></td>
				</tr>
				...
				<tr>
					...
					<td><input type="password" name="repassword" id="repassword">
						<span id="repassword_span"></span></td>
				</tr>

				<tr>
					<td>昵称:</td>
					<td><input type="text" name="nickname" id="nickname">
						<span id="nickname_span"></span></td>
				</tr>
	...
	<script type="text/javascript">
		function change() {
 			var key = document.getElementById("im");
 			key.src="${pageContext.request.contextPath }/checkImg?date=" + new Date();
		}
		//对用户名进行校验，验证用户名是否存在
		function lostfocus() {
			//javascript中 所有的变量都是var定义的
			var f=false;
			//获取用户输入的用户名
			...
			if(nname == ""){
				...
				document.getElementById("username_span").innerHTML = "用户名长度为6到16位";
			}else{
				...
				$.ajax({
					...
					success:function(data){
						if(data.msg == "false"){
							document.getElementById("username_span").innerHTML = "用户名已经存在";
							f=false;
						}else{
							document.getElementById("username_span").innerHTML = "";
							f=true;
		...
		function checkEmail() {
			...
			if(f == false){
				document.getElementById("email_span").innerHTML = "请输入正确邮箱";
				return false;
			}else{
